...
function resolveChartFile(filename) {
    if (!currentChartsPath || !fs.existsSync(currentChartsPath))
        return null;
...
function readDeclaredScheme(mbtiles, callback) {
    ...
    mbtiles._db.get("SELECT value FROM metadata WHERE name = 'scheme' LIMIT 1", (err, row) => {
        if (err)
            return callback(err);
...
const supabase = process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY
    ? createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { realtime: { transport: WebSocket } })
    : null;
...
app.get('/tiles/:filename/:z/:x/:y.png', (req, res) => {
    ...
    if (!mbtilesFile)
        return res.status(404).send('Carta no encontrada en el repositorio');
...
function sanitizeFunctionCalls(functionCalls, enabledToolNames) {
    ...
    return functionCalls.flatMap((call) => {
        if (!call || typeof call.name !== 'string' || !enabledToolNames.has(call.name))
            return [];
