...
// Corrección de iconos por defecto de Leaflet para entorno Electron/Vite
delete (L.Icon.Default.prototype as any)._getIconUrl;
L.Icon.Default.mergeOptions({
  iconUrl: '/barco-tactico.png',
});
      ...
      >
        ...
        {shipPosition && (
          <Marker 
            ...
            icon={L.icon({ iconUrl: 'barco-player.png', iconSize: [36, 36], iconAnchor: [18, 18], popupAnchor: [0, -18] })}
          >
            <Popup className="custom-popup">
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 w-48 shadow-2xl text-white">
                ...
                <p className="text-sm font-bold uppercase">{shipName}</p>
              </div>
            </Popup>
          </Marker>
        )}
        ...
        {targetDestination && (
          <Marker 
            ...
            icon={L.divIcon({ className: 'target-dest-icon', html: '<div class="w-8 h-8 bg-red-600 rounded-full flex items-center justify-center border-2 border-white shadow-lg animate-bounce">📍</div>', iconSize: [32, 32], iconAnchor: [16, 32] })}
          />
        )}
