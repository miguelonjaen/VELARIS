          ...
          >
            <div className="p-1 bg-gradient-to-b from-cyan-500/10 to-transparent border-b border-white/5">
              <div className="relative flex justify-center mb-1">
                <img src="/logo.png" alt="VELARIS" className="w-50 h-50 object-contain" />
              </div>
            ...
            <div className="p-8 space-y-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
              <p className="text-sm font-medium text-slate-300 leading-relaxed">{data.summary}</p>

              <div className="space-y-4">
                {data.sections.map((section, idx) => (
                  <div
                    key={`${section.title}-${idx}`}
                    className="flex gap-4 p-4 bg-white/5 border border-white/5 rounded-2xl group hover:border-cyan-500/20 transition-all"
                  >
                    <span className="text-2xl">{section.icon}</span>
                    <div className="flex-1">
                      <p className={cn('text-[8px] font-black uppercase tracking-widest mb-2', 'text-cyan-400')}>
                        {section.title}
                      </p>
